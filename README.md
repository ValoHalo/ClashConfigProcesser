...
- 自动选择组的检测参数见“DNS 与测速”。全量版固定出口主备组使用 Apple 测试页，预期 HTTP 200，默认间隔 400 秒、超时 3000 毫秒、失败检查阈值 2。国内与直连 DNS 使用 DoH，直连解析不加入系统 DNS。
...
| 快速故障恢复     | 关     | 固定出口主备组默认每 400 秒检测，启用后每 120 秒检测；严格固定组不定时换节点 |
...
关闭某个服务的固定出口开关后，该服务恢复完整候选列表。关闭服务本身时，同时移除本服务的固定出口与专用 DNS 策略。
...
全量版与精简版的自动选择组使用 `https://www.gstatic.com/generate_204`，检测周期为 60 秒，超时为 3 秒，预期 HTTP 状态码为 204。检测在策略组闲置时继续执行。延迟容差为 150 毫秒：当前节点可用时，候选节点的延迟优势超过该值才切换；当前节点不可用时，切换不受该容差限制。
...
- 使用脚本 DNS 时关闭客户端 DNS 覆写。保留各策略组的测速地址与预期状态码：自动组使用 Google generate_204（204），全量版固定出口主备组使用 Apple 测试页（200）。客户端测速地址和预期状态码应与对应策略组保持一致。
...
- 全量修改版包含 DNS、服务策略组和网络暴露设置；精简版及静态配置主要跟随上游
