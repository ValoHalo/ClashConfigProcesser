...
function runIntegrationTests(h, api, meta, fx, loadScript, scriptFile) {
  ...
  if (meta.full) {
    require('./full-dns').runFullDnsTests(h, api, meta, fx);
  } else {
    // ---------------- DNS 与 hosts ----------------
    h.section('集成测试 · DNS 与 hosts');
    h.test('proxy-server-nameserver 固定使用公共 DNS；无专属策略时私有 DNS 合并写入节点域名 policy', () => {
      ...
      // proxy-server-nameserver 固定使用公共 DNS，不再承载私有 DNS
    ...
    h.test('无 dns/hosts 输入时生成默认配置', () => {
      ...
      const out = api.main(cfg);
      h.assertEqual(out.dns.enable, true);
      // 默认 proxy-server-nameserver 使用公共 DNS；无私有 DNS 时不生成 policy
      h.assertDeep(out.dns['default-nameserver'], api.defaultDNS);
      h.assertDeep(out.dns['proxy-server-nameserver'], api.proxyServerDNS);
